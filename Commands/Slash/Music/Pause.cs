...
{
    public class PauseSlashCommand : ISlashCommands
    {
        public string CommandName => "pause";
        private readonly AudioService _audio;
        private readonly IConfiguration _configuration;

        public PauseSlashCommand(IConfiguration configuration, AudioService audio)
        {
        ...
        public async Task RegisterCommandsAsync(DiscordSocketClient client)
        {
            var cmd = new SlashCommandBuilder()
                ...
                .WithDescription("Pause the currently playing music.");
        ...
        public async Task HandleCommand(SocketSlashCommand command, DiscordSocketClient client)
        {
            ...
            {
                await command.RespondAsync("⏸️ Music is already paused.");
                return;
            }

            try
            {
                await _audio.PauseAsync(guildId);

                var embed = new EmbedBuilder()
                    .WithTitle("⏸️ Music Paused")
